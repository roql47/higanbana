...
export const SITES: Record<string, Site> = {
  ...
  /**
   * 폐여관 — 광장 남동 골목. **두 번 넓혔다.** 9×7 → 12×9 (객실이 벽장이었다) → **19×15**.
   * 두 번째 확장의 근거는 기믹이 크기를 요구했기 때문이다: 이 건물 하나가 「현실 + 거울 세계
   * 두 벌」에 「인파 셋의 시선 이벤트」까지 얹는데, 실내 108 m² 는 **전체 로케이션 중 최소**였고
   * (저택 140 · 폐교 378) 그 탓에 시선 반경을 2.2 m 로 깎아야 옆방까지 안 닿았다(v5.29 실측).
   * 복도 잔해도 통로를 사실상 막아 두 번 고쳤다(v5.15·v5.17) — 전부 폭이 없어서 난 문제다.
   * 동·남으로만 키운다: 서·북은 마츠리 광장이 1 m 앞이다.
   */
  inn: site('inn', 49.5, 50, 24, 20),                 // 실내 19×15 (285 m²)
  // 22×14(실내 깊이 9)로는 교실이 3.3 m 벽장이 됐고(v5.9에서 27×17), 사용자 「학교는 더 컸으면」으로
  // 한 번 더 — 32×19: 건물 27×14, 교실 깊이 5.6 · 남측을 교무실/준비실/방송실 세 칸으로 나눌 폭이 나온다
  school: site('school', 62, 7, 32, 19),              // 폐교 — 동쪽 대숲 너머
  manor: site('manor', 35, -29, 19, 15),              // 촌장 저택 — 신사 동편 높은 곳. 실내 14×10
   ...
   * 신사 경내 — 북쪽 정점.
   *
   * ⚠️ z 는 **경내 건물이 실제로 서는 자리**에서 나와야 한다. `Shrine` 은 자기 중심을
   * 「참배로 끝 − 9 m」(= z −58)로 잡는데, 예전 선반은 z −47 중심이라 **11 m 남쪽**에 깔려 있었다.
   * 그 결과 배전 뒷벽(z −63)이 평탄 구간(−60) 밖 산자락에 1.1 m 파묻히고, 돌바닥 정점색은
   * 정작 건물이 없는 앞마당 20 m 에만 칠해졌다 (2026-08-26 실측).
   * 중심을 −52 로 올려 z −65~−39 를 덮는다: 배전(−63.25)까지 선반 위, 본전(−66.5)은 선반 밖
   * 산자락에 **일부러** 남긴다 — 본전은 배전보다 높아야 하고, `shrine.ts` 가 그 높이를 따로 잰다.
   */
  shrine: site('shrine', 0, -52, 32, 26),
  // 무연불 묘지 — 참사 희생자 87명 × 통로 포함 약 12 m²에서 반경 약 18.2 m 를 역산한다.
  // 중심 선반은 32×28만 완만하게 다듬고 외곽 묘열은 산자락 경사로 흘려, 사각 공터처럼 보이지 않게 한다.
  // 초칭 사거리 15 m보다 넓으므로 중앙에서도 가장자리의 일부는 계속 어둠에 잠긴다.
  graveyard: site('graveyard', -40, 15, 32, 28, 0.55, 5),
  // 할머니 집 부지와 겹치지 않는 동쪽 빈터. 옛 좌표(-25, 29)는 집·마루 안까지 꽃밭에 포함됐다.
  flower: site('flower', -17.5, 34.5, 13, 12, 0, 3),  // 피안화 군락 — 지형은 그대로
...
export const ROUTES: Route[] = [
   ...
   * 광장에서 저택을 지나 신사 뒤로.
   *
   * ⚠️ 경유점이 `SITES.manor` 와 **같은 좌표(35, −29)**, 즉 저택 정중앙이었다. 부지 좌표를
   * 그대로 웨이포인트로 쓰면서 건물 폭(14×10)을 안 뺀 탓이다 — 자갈길이 저택 바닥을 대각선으로
   * 가로질러 **건물 바닥 표본의 17 %가 노면**이었고, 실내에서 자갈 발소리가 났다(2026-08-26 실측).
   * 문이 서쪽(x−)이므로 길을 서쪽으로 10 m 빼서 **정문 앞을 지나게** 한다.
   */
  { id: 'stair', name: '돌계단 뒷길', surface: 'gravel', halfWidth: 1.0, blend: 1.2, flatten: 0.9,
    pts: [[31, 24], [33, 8], [31, -10], [25, -29], [22, -40], [8, -47]] },
  // 동쪽 대숲 — 여관과 폐교로
