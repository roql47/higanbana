...
export class WellCinematics {
  ...
  surfaceOutro(player: THREE.Vector3, well: THREE.Vector3, speaker: THREE.Vector3): Sequence {
    ...
    const horn = speaker.clone().addScaledVector(toWell.right, 1.4).addScaledVector(toWell.forward, -1.2).add(new THREE.Vector3(0, 0.3, 0));
    return {
      id: 'well-surface-outro', duration: 7.4, skippable: true,
      cam: [
        this.key(0, low, groundLook, 50),
        this.key(2.6, mouth, well.clone().add(new THREE.Vector3(0, 0.55, 0)), 47),
        this.key(5.0, horn, speaker, 44),
        this.key(7.4, horn.clone().add(new THREE.Vector3(0, 0.12, 0)), speaker, 46),
      ],
    };
  }
}
