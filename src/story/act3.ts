 ...
 *   · 비석을 다 닦은 순간부터 마지막 대사가 끝날 때까지 조작을 회수한다. 카메라와 몸이
 *     소리 나는 쪽으로 돌아가고, 미오는 **한 번의 부름에** 대답한다
 ...
 *
 * 셋 다 **끌기**(`cam.pull`)다. 입력은 main 에서 막고, 몸은 **걷고 있지 않을 때만** 돌린다.
 */
...
export class Act3 {
  ...
  private stainT = -1;
  /** 마지막 얼룩 대사를 큐에 넣었는가 — 이 대사가 사라져야 제어권을 돌려준다. */
  private finalDialogueQueued = false;
  ...
  constructor(private d: Act3Deps) {
    ...
    this.beats = [
      { at: 0.0, run: (d) => void d.dialogue.say({ text: L('이끼를 닦아내자 글자가 드러난다 — 히가사토의 세 가지 금기.', '苔を拭うと文字が現れる — 彼ヶ里 三禁。'), dur: 2.7 }) },
  ...
  /** 이동·카메라·행동 입력을 모두 막아야 하는 구간. */
  get controlsLocked() { return this.running; }
  /** main 이 이동 입력에 곱한다 */
  get moveScale() { return this.controlsLocked ? 0 : 1; }
  /** DEV */
  ...
  update(dt: number) {
    ...
    //    낭독이 시작되면 제어권을 회수하므로, 입력과 카메라 연출이 서로 싸우지 않는다.
    //    거리 조건은 디버그 텔레포트처럼 위치가 강제로 바뀐 경우의 안전장치다.
    ...
    if (this.state === 'call') {
      this.ct += dt;
      // 시선이 목소리 쪽으로 끌린다. 이 구간에는 마우스 입력도 잠겨 있다.
      ...
      // 연출 카메라가 목소리 쪽에 도착했는가 — 도착하면 첫 부름 뒤 대답한다
      ...
      if (this.stainT >= 0) {
        ...
        // 얼룩이 다 번지고 마지막 대사까지 끝난 뒤에만 제어권을 돌려준다.
        if (this.stainT >= STAIN_S && this.finalDialogueQueued && !d.dialogue.busy) this.finish();
      }
    }
  ...
  /** 첫 부름 — 「미오야.」 멀리서. 제어권은 비석 낭독 시작부터 이미 잠겨 있다. */
  private call1() {
    this.state = 'call';
    this.ct = 0;
    const d = this.d;
  ...
  /** 두 번째 부름 — 연출 카메라가 아직 목소리 쪽에 도착하지 않았을 때 더 가까이서 들린다. */
  private call2() {
    this.call2Done = true;
    const d = this.d;
