...
ROOT=Path('/Users/jay/Claude/3D_motion');OUT=ROOT/'artifacts/ogimachi-phases/runtime-irori-interior-v3';OUT.mkdir(parents=True,exist_ok=True)
...
def classify(o):
 ...
 if o.get('runtimeGroup') in ['interior','cushions']:return o['runtimeGroup']
...
def execute():
 try:
  bpy.context.window.scene=scene
  for name in ['sign','body','roof','garden','interior','cushions','doorLeft','doorRight']:
  ...
  root=bpy.data.objects.new('irori-restaurant',None);root['archetype']='irori-restaurant';root['osm_id']=236248710;root['runtime_revision']='interior-v3';scene.collection.objects.link(root)
