...
ROOT=Path('/Users/jay/Claude/3D_motion');OUT=ROOT/'artifacts/ogimachi-phases/runtime-irori-v6';OUT.mkdir(parents=True,exist_ok=True)
...
scene=bpy.data.scenes.new('Irori Runtime V6 Bake');scene.render.engine='CYCLES';scene.cycles.samples=8;scene.cycles.device='GPU';scene.render.bake.margin=8;scene.render.bake.use_clear=True;scene.render.bake.use_pass_direct=False;scene.render.bake.use_pass_indirect=False;scene.render.bake.use_pass_color=True
...
for name,objects in groups.items():
 ...
 ob=objects[0];ob.name='irori-v6-'+name;merged[name]=ob
 ...
 try:
  ...
  for name in ['sign','body','roof','garden']:
   ...
   for kind,bake_type in [('color','DIFFUSE'),('roughness','ROUGHNESS'),('normal','NORMAL')]:
    ...
    im=bpy.data.images.new('Irori v6 '+name+' '+kind,size,size,alpha=False);im.colorspace_settings.name='sRGB' if kind=='color' else 'Non-Color'
  ...
  root=bpy.data.objects.new('irori-restaurant',None);root['archetype']='irori-restaurant';root['osm_id']=236248710;root['runtime_revision']='photoreal-v6';scene.collection.objects.link(root)
  ...
  bpy.ops.export_scene.gltf(filepath=str(OUT/'irori-restaurant-v6.glb'),export_format='GLB',use_selection=True,use_active_scene=True,export_apply=True,export_extras=True)
  bpy.data.libraries.write(str(OUT/'irori-runtime-v6.blend'),{scene},fake_user=True,compress=True)
  report.update(status='complete',bytes=(OUT/'irori-restaurant-v6.glb').stat().st_size);(OUT/'status.json').write_text(json.dumps(report,indent=2))
 except Exception:
