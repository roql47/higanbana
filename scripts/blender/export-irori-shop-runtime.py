...
ROOT=Path('/Users/jay/Claude/3D_motion');OUT=ROOT/'artifacts/ogimachi-phases/runtime-irori-shop-v1';OUT.mkdir(parents=True,exist_ok=True)
source=bpy.context.scene;building=next(o for o in source.objects if o.get('osm_id')==236248704)
assert not bpy.app.is_job_running('RENDER')
scene=bpy.data.scenes.new('Irori Shop Bake');scene.render.engine='CYCLES';scene.cycles.samples=8;scene.cycles.device='GPU';scene.render.bake.margin=8;scene.render.bake.use_clear=True;scene.render.bake.use_pass_direct=False;scene.render.bake.use_pass_indirect=False;scene.render.bake.use_pass_color=True
...
def execute():
 try:
  ...
  root=bpy.data.objects.new('irori-shop',None);root['archetype']='irori-shop';root['osm_id']=236248704;root['runtime_revision']='photoreal-v5';scene.collection.objects.link(root)
  ...
  bpy.ops.export_scene.gltf(filepath=str(OUT/'irori-shop-v5.glb'),export_format='GLB',use_selection=True,use_active_scene=True,export_apply=True,export_extras=True)
  bpy.data.libraries.write(str(OUT/'irori-shop-runtime-v1.blend'),{scene},fake_user=True,compress=True)
  report.update(status='complete',bytes=(OUT/'irori-shop-v5.glb').stat().st_size);(OUT/'status.json').write_text(json.dumps(report,indent=2))
 except Exception:
