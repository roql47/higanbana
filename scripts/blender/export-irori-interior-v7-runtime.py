...
ROOT=Path('/Users/jay/Claude/3D_motion');OUT=ROOT/'artifacts/ogimachi-phases/runtime-irori-interior-v7-packed';OUT.mkdir(parents=True,exist_ok=True)
...
def visible(o):
 ...
 return True
def classify(o):
 if o.name.startswith(('IR6 ','IR interior wall post','IR interior ceiling cross beam','IR2 wall rail','IR2 wainscot batten','IR2 front mullion')):return 'timber'
 if o.get('iroriDoorSide'):
  return ('doorGlass' if o.get('iroriDoorGlass') else 'door')+('Left' if o['iroriDoorSide']==1 else 'Right')
 if o.get('runtimeGroup') in ['interior','cushions','tatami','floor','cabinet']:return o['runtimeGroup']
...
def execute():
 try:
  bpy.context.window.scene=scene
  for name in ['sign','body','roof','garden','interior','cushions','tatami','floor','timber','cabinet','doorLeft','doorRight']:
   ...
   bpy.ops.object.mode_set(mode='EDIT');bpy.ops.mesh.select_all(action='SELECT');bpy.ops.uv.smart_project(angle_limit=math.radians(66),island_margin=.0025,margin_method='FRACTION',area_weight=1.0,correct_aspect=True,scale_to_bounds=True);bpy.ops.object.mode_set(mode='OBJECT')
   size=1024 if name in ['sign','doorLeft','doorRight'] else 2048;maps={}
   ...
   if name in ['floor','cabinet','interior','tatami','cushions','timber']:
    report.update(status='baking',group=name,map='occlusion');(OUT/'status.json').write_text(json.dumps(report,indent=2))
    im=bpy.data.images.new('Irori contact '+name,size,size,alpha=False);im.colorspace_settings.name='Non-Color';saved=[]
    for srcmat in ob.data.materials:
     if not srcmat or not srcmat.use_nodes:continue
     nt=srcmat.node_tree;out=next(n for n in nt.nodes if n.type=='OUTPUT_MATERIAL');links=list(out.inputs['Surface'].links);previous=links[0].from_socket if links else None
     ao=nt.nodes.new('ShaderNodeAmbientOcclusion');ao.inputs['Distance'].default_value=.38;ao.samples=16
     em=nt.nodes.new('ShaderNodeEmission');nt.links.new(ao.outputs['Color'],em.inputs['Color']);nt.links.new(em.outputs[0],out.inputs['Surface'])
     target=nt.nodes.new('ShaderNodeTexImage');target.image=im;nt.nodes.active=target;saved.append((nt,out,previous,ao,em,target))
    bpy.ops.object.bake(type='EMIT')
    for nt,out,previous,ao,em,target in saved:
     nt.nodes.remove(ao);nt.nodes.remove(em);nt.nodes.remove(target)
     if previous:nt.links.new(previous,out.inputs['Surface'])
    im.filepath_raw=str(OUT/(name+'-occlusion.png'));im.file_format='PNG';im.save()
  ...
  root=bpy.data.objects.new('irori-restaurant',None);root['archetype']='irori-restaurant';root['osm_id']=236248710;root['runtime_revision']='interior-v7';scene.collection.objects.link(root)
  for name,o in merged.items():
   o.parent=root
   if name.startswith('door'):o['iroriDoorSide']=1 if name.endswith('Left') else -1
  for original in building.children_recursive:
   if original.type=='EMPTY' and (original.get('collisionBox') or original.get('iroriLight')):
    ob=bpy.data.objects.new(original.name,None);scene.collection.objects.link(ob);ob.parent=root;ob.matrix_local=building.matrix_world.inverted()@original.matrix_world
    for key in ['collisionBox','walkSurface','iroriLight']:
     if key in original:ob[key]=original[key]
