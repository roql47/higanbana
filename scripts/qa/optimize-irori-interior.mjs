...
const doc=await io.read('artifacts/ogimachi-phases/runtime-irori-interior-v1/irori-restaurant-v5.glb');
...
for(const [key,count] of [['collisionBox',19],['iroriLight',4],['iroriDoorSide',4]]) {
 if(root.listChildren().filter(n=>n.getExtras()[key]).length!==count)throw Error(`Missing runtime metadata: ${key}`);
}
await io.write('public/models/ogimachi/irori-restaurant-interior-v1.glb',doc);
